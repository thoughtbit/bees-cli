...
var argv = require('yargs').usage('Usage: bee server [options]').option('open', {
  type: 'boolean',
...
function readRcConfig() {
  try {
    ...
    console.log(_chalk2.default.red('Failed to parse .beerc config.'));
    console.log();
...
function setupWatch(devServer) {
  var files = [paths.resolveApp('.beerc'), paths.resolveApp('.beerc.js'), paths.resolveApp('webpack.config.js')].concat(typeof rcConfig.theme === 'string' ? paths.resolveApp(rcConfig.theme) : []);
