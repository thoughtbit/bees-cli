...
function readRcConfig() {
  ...
  if (!rcConfig.use) {
    console.log(_chalk2.default.red('use config not found in .beesrc'));
    process.exit(1);
  }
}
...
function setupWatch(devServer) {
  var files = [paths.resolveApp('.beesrc'), paths.resolveApp('.beesrc.js'), paths.resolveApp('webpack.config.js')].concat(typeof rcConfig.theme === 'string' ? paths.resolveApp(rcConfig.theme) : []);
