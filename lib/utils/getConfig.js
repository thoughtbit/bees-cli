...
exports.default = function (env, cwd) {
  ...
  return realGetConfig('.beesrc', env, pkg, paths);
};
...
function merge(oldObj, newObj) {
  for (var key in newObj) {
    if (Array.isArray(newObj[key]) && Array.isArray(oldObj[key])) {
      ...
      oldObj[key] = (0, _assign2.default)(oldObj[key], newObj[key]);
    } else {
...
function getConfig(configFile, paths) {
  ...
  if ((0, _fs.existsSync)(rcConfig)) {
    // return parseJSON(stripJsonComments(readFileSync(rcConfig, 'utf-8')), './beesrc')
...
function realGetConfig(configFile, env) {
  ...
  return (0, _keys2.default)(config).reduce(function (memo, key) {
